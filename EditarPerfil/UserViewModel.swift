...
class UserViewModel: ObservableObject {
    ...
    let userid = "5b0d4074664528131833c5684f2e2d0c"
    ...
    func getUser() {
        let task = URLSession.shared.dataTask(with: URL(string: apiurl + "/get-user/?_id=\(userid)")!) { data, _, error in
            do {
    ...
    func put() {
        ...
        do{
            var json = try JSONEncoder().encode(self.user!)
            print(json)
